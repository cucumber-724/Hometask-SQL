...
FROM Треки 
ORDER BY Длительность_трека DESC 
LIMIT 1;

SELECT Название_трека, Длительность_трека 
FROM Треки 
WHERE Длительность_трека >= '00:03:30';

SELECT Название_сборника, Год_выпуска 
FROM Сборники 
...
FROM Треки 
WHERE Название_трека ILIKE '%мой%' OR Название_трека ILIKE '%my%';
...
WHERE и.Имя_исполнителя = 'Billie Eilish'
...
GROUP BY а.Id_Альбома, а.Название_альбома
HAVING COUNT(DISTINCT иж.Id_Жанра) > 1;
